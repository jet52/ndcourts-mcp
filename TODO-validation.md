...
- [ ] Hand-resolve the 29 final `merge_westlaw_text` residuals. ~4 are wrong pairings (delete the `opinion_sources` row), ~23 are length-rejects where Westlaw extracts only the opinion body but the DB has body + CL-OCR headnotes (decide per-case whether the body-only Westlaw text is preferable). 2 are missing both an Opinion header and Attorneys section — likely unrecoverable from this Westlaw export.
