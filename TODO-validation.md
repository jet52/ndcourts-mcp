...
- [x] **Investigate the remaining 93 diff-audit <0.20 archive pairs**. **Done 2026-05-04** — characterized in `triage/archive-83-deep-triage-2026-05-04.md`. 10 of the 93 are westlaw+NW (separate item below); the 83 archive pairs split into five distinct failure modes:
  ...
  - [x] **Type Y / contaminated citations (5)**: 3 of 5 fixed 2026-05-04 via `fix_type_y_archive_pairings` (batch `fix-type-y-archive-pairings-2026-05-04`, 6 changelog rows). The Keller pair (13202↔13203) was a one-way move; the two Johnson rows (15805↔15807) were a mutual swap — both opinions had each other's archive linked. Stray neutral cites also removed from the lead rows. The remaining 2 are deeper data bugs:
    - [ ] **Kleinsmith / Kitchen (oid 16488 + 15988)**: archive HTML titled "Reciprocal Discipline of Kitchen, 2013 ND 18, 826 N.W.2d 620" but both DB rows are recorded as "Kleinsmith". Either the DB case_name is mis-recorded (and the archive is correctly the Kleinsmith/Kitchen opinion at 2013 ND 18) or Kitchen is a third opinion not in the DB. Read `markdown/2013/2013ND18.md` and `2013ND19.md` to determine which case each row actually represents, then re-link or insert.
    - [ ] **Feldmann (oid 16829)**: row's `source_path` points at the wrong markdown file (`markdown/2017/2017ND255.md` instead of `markdown/2017/2017ND1.md`), so `text_content` is also contaminated — the entire row's identity is wrong. Needs a full row repair: re-extract text_content from the correct markdown, fix source_path, drop stray cite `2017 ND 255`, detach the wrong archive linkage. The corresponding correct row (oid 17060) for 2017 ND 255 already exists.
