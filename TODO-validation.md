...
- [ ] **Investigate the remaining 93 diff-audit <0.20 archive pairs** — these have archive titles that DO match the DB citation but their body text disagrees with the primary source by >80%. Likely cases: truncated archive HTML, stub/error pages saved by the archive scraper, or DB primary text quality issues that make the pair look mismatched even when the file is correct. Sample-investigate before deciding the right fix (re-scrape, detach, accept).
