...
- **Pending (CourtListener re-auth):** **20482** *(298 N.W.2d 372, 1980 per curiam arson disbarment by consent)* — stored text is only a Westlaw synopsis stub and never names the attorney, so `case_name` is still the garbage `"Unk"`. Set `per_curiam=1` and `crosscheck_state='flagged'` with a note; caption recovery deferred to a CourtListener lookup. **9770** *Estate of Papineau v. All Other Persons Unknown* is a false positive — "Unknown" is a legitimate quiet-title defendant; no change.
